"""
AD5933 — Pair A, Code 1: Measure + Save to Pico Flash
======================================================
Loads calibration from cal_data.txt (written by cal_extract_savesafile.py).
Runs impedance sweep, saves results to result.csv on Pico flash.
On laptop run:  python fetch_and_plot_pico.py

FIXES applied vs previous version:
  1. _freq_code() now uses (f * 4 / MCLK_HZ) * 2^27 — correct datasheet formula.
...
RESULT_FILE = "result.csv"     # output saved to Pico flash
...
# FIX 1: multiply by 4 — datasheet formula: Code = (f_out / (MCLK/4)) * 2^27
...
print("\n AD5933 — Measure + Save to Pico Flash")
print("  I2C:", i2c.scan())
...
if not raw:
    ...
    count = 0
    # Open file once, write line-by-line to avoid RAM overflow on Pico
    with open(RESULT_FILE, "w") as out:
        ...
        for idx, (freq, r, im) in enumerate(raw):
            ...
            # FIX 2: atan2 returns correct signed angle for all quadrants (−pi, pi].
            #         Previous manual quadrant correction was wrong for Q2/Q3
            #         and produced incorrect phase for reactive loads (capacitors).
            z_phase_rad = math.atan2(im, r) - sp
            ...
            print("     freq,   "" z_mag, "      "    z_phase_deg, "          " z_real, "      "     z_imag")
            print("  {:>8d} {:>12.2f} {:>10.3f} {:>12.2f} {:>12.2f}".format(
                freq, z_mag, z_phase_deg, z_real, z_imag))
            count += 1

    print("\n  Saved {} points to \'{}\'.".format(count, RESULT_FILE))

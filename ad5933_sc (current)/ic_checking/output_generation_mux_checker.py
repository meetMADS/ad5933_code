...
_SHRT = machine.Pin(0,  machine.Pin.OUT)   # GP0
_ASL0 = machine.Pin(1,  machine.Pin.OUT)   # GP1
_ASL1 = machine.Pin(2,  machine.Pin.OUT)   # GP2
_BSL0 = machine.Pin(6,  machine.Pin.OUT)   # GP6
_BSL1 = machine.Pin(7,  machine.Pin.OUT)   # GP7
_SW_DUT_RCAL = machine.Pin(28, machine.Pin.OUT) # Low = Rcal, High = DUT
...
def switching_logic_rcal_rfb(idx):
    ...
    print("R[{}] = {} kΩ — select lines: {}".format(
        idx, r_known[idx] / 1e3, r_select_lines[idx]))
...
# DWELL_MS = 2000   # change to however long you need the tone on each R
...
# print("Sinusoid active on VOUT. Cycling through resistors...")

# idx = 0
# while True:
#     switching_logic_rcal_rfb(idx)           # set mux to current R
#     time.sleep_ms(DWELL_MS)                 # hold for measurement
#     idx = (idx + 1) % len(r_select_lines)   # advance, wrap at end

# ── INDEX MODE: 1-based (user types 01 to 12) ────────────────────────────────
# Change to zero-based by removing the -1 offset below if preferred.


current_idx = 0
switching_logic_rcal_rfb(current_idx)   # apply default on boot

while True:
    print("\nCurrent R: {} kΩ  (index {})".format(
        r_known[current_idx] / 1e3, current_idx + 1))
    print("Enter index (1–12) or 'e' to exit:")

    cin = input(">> ").strip()

    if cin.isdigit():
        idx = int(cin)
        if 1 <= idx <= len(r_select_lines):
            current_idx = idx - 1
            switching_logic_rcal_rfb(current_idx)
            print("Switched → {} kΩ".format(r_known[current_idx] / 1e3))
        else:
            print("Invalid. Enter 1 to {}.".format(len(r_select_lines)))
